...
void pushup(int p, int l, int r)
{
	if(l < r)
	{
		...
		tree[p].is91 = tree[DXA(p)].is91 & tree[DXB(p)].is91;
	}
}
...
{
	if(tree[p].is91) return ;
	if(nl == nr)
	{
		tree[p].sum = tree[p].sum > 100 ? tree[p].sum - 10 : 91;
		tree[p].is91 = tree[p].sum == 91;
		return ;
	}
	int mid = midf(nl, nr);
