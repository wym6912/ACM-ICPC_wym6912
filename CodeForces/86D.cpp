...
int main()
{
	...
	for(int i = 1; i <= n; i ++)
		scan_d(a[i]);
	memcpy(data + 1, a + 1, n * sizeof(ll));
	sort(data + 1, data + 1 + n);
	int p = unique(data + 1, data + 1 + n) - data;
	for(int i = 1; i <= n; i ++)
		a[i] = lower_bound(data + 1, data + p, a[i]) - data;
	for(int i = 1; i <= m; i ++)
	{
		node[i].id = i;
		scan_d(node[i].L);
